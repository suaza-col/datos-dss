...
# Formato: iso3, NAME_2, cod_local, anio, sexo, zona, etnia, valor
...
base <- base %>%
  ...
  mutate(
    ...
    mejora_postpandemia = case_when(
      anio >= 2023 & tipo_zona == "urbano"     ~ -0.010,
      anio >= 2023 & tipo_zona == "periurbano" ~ -0.008,
      TRUE ~ 0
    ),
    ...
    valor_individual = pmin(pmax(valor_individual, 0.02), 0.95),
...
message("✅ journey_time.parquet guardado")
